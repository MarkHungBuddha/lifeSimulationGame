...
export function SimulatorApp() {
  ...
  const advancedSettings = (
    <Box sx={{ width: isMobile ? '100%' : DRAWER_WIDTH, height: '100%', overflowY: 'auto', WebkitOverflowScrolling: 'touch', bgcolor: 'background.default' }}>
      <Controls />
    </Box>
  )

  const advancedDrawerContent = (
    <Box sx={{ maxHeight: isMobile ? 'calc(100dvh - 56px)' : '100dvh', display: 'flex', flexDirection: 'column' }}>
      <Box
        sx={{
          px: 2,
          py: 1,
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default' }}>
      <AppBar position="fixed" sx={{ zIndex: (t) => t.zIndex.drawer + 1 }} elevation={0}>
        <Toolbar sx={{ minHeight: { xs: 56, sm: 64 }, gap: 1.25, px: { xs: 2, sm: 3, md: 6 } }}>
          ...
          >
            {isSmall ? '' : t('mobile_action.adjust')}
          </Button>
        </Toolbar>
      </AppBar>
      ...
      <Drawer
        ...
        sx={{
          '& .MuiDrawer-paper': {
            ...
            borderLeft: isMobile ? 0 : '1px solid',
            borderColor: 'divider',
          },
        }}
      >
      ...
      >
        <GuidedSimulator
          ...
          startOnResults={searchParams.get('autoRun') === '1'}
        />
      </Box>
    </Box>
  )
